...
const Cart = () => {
    ...
    if (typeof window !== "undefined" && window.localStorage) {
      ...
      if (storedCart) {
        ...
        storedCart.forEach((item: IProduct) => {
          totalCart += parseFloat(item.price);
        });
        setTotal(totalCart);
        setCart(storedCart);
      }
  ...
  const handleRemoveFromCart = (productId: string | undefined) => {
    ...
    // Recalcular el total
    const updatedTotal = updatedCart.reduce((acc, curr) => acc + parseFloat(curr.price), 0);
            ...
            cart.map((product) => (
              <div key={product.id} className="flex items-center bg-gray-50 p-4 rounded-lg shadow-sm">
                ...
                <button
                  ...
                  className="bg-gray-500 hover:bg-gray-300 text-white p-2 rounded-md ml-28"
                >
                  Remove
                </button>
              </div>
